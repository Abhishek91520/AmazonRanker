...
function hasProxyFallbackConfig(): boolean {
  return Boolean(process.env.ZENROWS_API_KEY || process.env.SCRAPINGBEE_API_KEY);
}

function buildProxyFetchUrl(targetUrl: string): { provider: string; url: string } | null {
  if (process.env.ZENROWS_API_KEY) {
    const apiKey = encodeURIComponent(process.env.ZENROWS_API_KEY);
    const encodedTarget = encodeURIComponent(targetUrl);
    return {
      provider: 'zenrows',
      url: `https://api.zenrows.com/v1/?apikey=${apiKey}&url=${encodedTarget}&js_render=true&premium_proxy=true`,
    };
  }
  ...
  if (process.env.SCRAPINGBEE_API_KEY) {
    ...
    return {
      ...
      url: `https://app.scrapingbee.com/api/v1/?api_key=${apiKey}&url=${encodedTarget}&render_js=true&premium_proxy=true`,
    };
  }
...
function parseRanksFromHtml(html: string, targetAsin: string): HtmlParseResult {
  ...
  const upperTarget = targetAsin.toUpperCase();
  ...
  while ((match = asinRegex.exec(html)) !== null) {
    ...
    const isSponsored = /\bsponsored\b|sp-sponsored|slredirect|\/gp\/slredirect\/|sspa|sp_csd|adsense|cel_widget_id[^>]*sp_|data-csa-c-slot-id[^>]*sp/.test(
      context
    );

    if (isSponsored) {
      sponsoredRankCounter++;
      if (asin === upperTarget && foundSponsoredRank === null) {
        foundSponsoredRank = sponsoredRankCounter;
      }
    } else {
      organicRankCounter++;
      if (asin === upperTarget && foundOrganicRank === null) {
  ...
  return {
    found: foundOrganicRank !== null || foundSponsoredRank !== null,
    organicRank: foundOrganicRank,
    sponsoredRank: foundSponsoredRank,
    totalOrganicCount: organicRankCounter,
    totalSponsoredCount: sponsoredRankCounter,
    totalResults,
  };
}

async function executeProxyFallbackSearch(
  asin: string,
  keyword: string,
  config: ScraperConfig,
  runId: string,
  debugLogger?: DebugLogger
): Promise<RankCheckResponse | null> {
  if (!hasProxyFallbackConfig()) {
    scraperLog(runId, 'Proxy fallback skipped (no provider config)');
    debugLogger?.('proxy_fallback_skipped_no_config');
    return null;
  }

  let cumulativeOrganic = 0;
  let cumulativeSponsored = 0;
  let totalScanned = 0;

  scraperLog(runId, 'Starting proxy fallback search');
  debugLogger?.('proxy_fallback_started', {
    maxPages: config.maxPages,
    provider: process.env.ZENROWS_API_KEY ? 'zenrows' : 'scrapingbee',
  });

  for (let pageNum = 1; pageNum <= config.maxPages; pageNum++) {
    const targetUrl = getLocationAwareSearchUrl(keyword, pageNum, undefined);
    const proxyTarget = buildProxyFetchUrl(targetUrl);
    if (!proxyTarget) {
      return null;
    }

    scraperLog(runId, 'Proxy fallback fetching page', {
      pageNum,
      provider: proxyTarget.provider,
      targetUrl,
    });
    debugLogger?.('proxy_fallback_page_fetch_started', {
      pageNum,
      provider: proxyTarget.provider,
      targetUrl,
    });
    ...
    if (!response.ok) {
      scraperLog(runId, 'Proxy fallback page fetch failed', {
        pageNum,
        status: response.status,
      });
      debugLogger?.('proxy_fallback_page_fetch_failed', {
        pageNum,
        status: response.status,
      });
      continue;
    ...
    const parsed = parseRanksFromHtml(html, asin);
    totalScanned += parsed.totalResults;

    scraperLog(runId, 'Proxy fallback page parsed', {
      pageNum,
      found: parsed.found,
      organicRankOnPage: parsed.organicRank,
      sponsoredRankOnPage: parsed.sponsoredRank,
      totalResultsOnPage: parsed.totalResults,
      totalOrganicCount: parsed.totalOrganicCount,
      totalSponsoredCount: parsed.totalSponsoredCount,
    });
    debugLogger?.('proxy_fallback_page_parsed', {
      pageNum,
      found: parsed.found,
      organicRankOnPage: parsed.organicRank,
      sponsoredRankOnPage: parsed.sponsoredRank,
    ...
    if (parsed.found) {
      const result: RankResult = {
        asin,
        keyword,
        organicRank: parsed.organicRank !== null ? cumulativeOrganic + parsed.organicRank : null,
        sponsoredRank:
          parsed.sponsoredRank !== null ? cumulativeSponsored + parsed.sponsoredRank : null,
        pageFound: pageNum,
        positionOnPage: null,
    ...
    cumulativeOrganic += parsed.totalOrganicCount;
    cumulativeSponsored += parsed.totalSponsoredCount;
  }

  scraperLog(runId, 'Proxy fallback finished without match', { totalScanned });
  debugLogger?.('proxy_fallback_failed', { totalScanned });
  return null;
}
...
async function executeSearch(
  ...
  try {
    ...
    // Final fallback strategy: proxy-backed serverless HTML fetch (ZenRows/ScrapingBee)
    const proxyFallbackResult = await executeProxyFallbackSearch(
      asin,
      keyword,
      config,
      runId,
    ...
    if (proxyFallbackResult) {
      return proxyFallbackResult;
    }
